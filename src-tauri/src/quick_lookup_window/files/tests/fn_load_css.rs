...
fn works_with_no_prefix() {
    ...
    mock_app_data_directory_manager
        ...
        .times(1)
        .return_const(Ok(Path::new("home/joytyping").to_path_buf()));

    mock_files_dependencies
        .expect_read_to_string()
        .with(eq(PathBuf::from("home/joytyping/build/bundle.css")))
        .times(1)
    ...
    assert_eq!(
        files.load_css(Some("build/bundle.css".to_string())).unwrap().unwrap(),"\
...
fn works_with_some_prefix() {
    ...
    mock_files_dependencies
        .expect_read_to_string()
        .with(eq(PathBuf::from("home/joytyping/layout/build/build/bundle.css")))
        .times(1)
        .returning(|_|Ok("css code placeholder".to_string()));
    ...
    let files = Files {
       dependencies: Box::new(mock_files_dependencies),
       app_data_directory_manager: Box::new(mock_app_data_directory_manager),
       path_prefix: Some(PathBuf::from("layout/build")),
    };
...
fn interprets_app_data_directory_manager_error() {
    ...
    assert_eq!(files.load_css(Some("build/bundle.css".to_string())).unwrap_err(),
...
fn interprets_dependencies_read_to_string_error() {
    ...
    assert_eq!(settings.load_css(Some("build/bundle.css".to_string())),
