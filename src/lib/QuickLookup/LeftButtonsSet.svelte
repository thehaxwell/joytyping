<script lang="ts">
  ...
  function buildDoubleCross(
    ...
    return [
    buildCrossPoints(startX,startY,SQ_LENGTH,labels.slice(4,8)),
    buildCrossPoints(startX+2*SQ_LENGTH,startY+SQ_LENGTH,SQ_LENGTH,labels.slice(0,4)),
    ]
  }

  const SPACING = 10;
  let polygons = [
    ...buildDoubleCross(4*SQ_LENGTH + SPACING,0,labels.slice(0,8)),
    ...buildDoubleCross(8*SQ_LENGTH + 1.5*SPACING,2*SQ_LENGTH + SPACING/2,labels.slice(8,16)),
    ...buildDoubleCross(SQ_LENGTH,SQ_LENGTH + SPACING/2,labels.slice(16,24)),
    ...buildDoubleCross(5*SQ_LENGTH + SPACING/2,3*SQ_LENGTH + SPACING,labels.slice(24,32)),
  ];
