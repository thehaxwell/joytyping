...
use crate::joy_keyboard::{input_controller::{enigo_wrapper::MockEnigoTrait, InputController, DELAY_DURATION}, keys_config::KeyClickConfig};

use super::InputControllerTrait;
...
//    JoyKeyboard::trigger_input(), once each time it is called on loop by
...
fn key_down_for_key_with_modifiers_works() {
    ...
    input_controller.key_down(
        KeyClickConfig{
            ...
            modifiers:[Some(enigo::Key::Shift),Some(enigo::Key::Alt),None,None,None]
        });
    assert_just_now(input_controller.instant_to_start_delay_from.unwrap());
}
